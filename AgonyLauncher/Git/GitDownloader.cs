...
namespace AgonyLauncher.Git
{
    public static class GitDownloader
    {
        public static bool Download(string url, string directory)
        {
            ...
            Log.Instance.DoLog(string.Format("Cloning repository: \"{0}\" to directory: \"{1}\".", url, directory));

            try
            {
                Repository.Clone(url, directory, new CloneOptions { Checkout = true });

                Log.Instance.DoLog(string.Format("Successfully cloned repository: \"{0}\" to directory: \"{1}\".", url, directory));
            }
            catch (Exception ex)
            {
                Log.Instance.DoLog(
