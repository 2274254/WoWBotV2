...
////////////////// PathFinder //////////////////
PathFinder::PathFinder(unsigned int mapId, unsigned int instanceId, bool allowSwimming) :
...
{
	Movement::Vector3 start(originX, originY, originZ);
	setStartPosition(start);

	Movement::Vector3 dest(destX, destY, destZ);
	setEndPosition(dest);
    ...
    if (m_type == PathType(PATHFIND_NOPATH) && !isSwimming) {
        printf("Doing water based recheck due to default pathing fail");
        updateFilter(true, originX, originY, originZ);
        BuildPolyPath(start, dest);
    }
...
void PathFinder::BuildPolyPath(const Movement::Vector3& startPos, const Movement::Vector3& endPos)
{
	...
	if (farFromPoly)
	{
		...
		else
		{
			...
			if (dtStatusSucceed(dtResult))
			{
				...
				setActualEndPosition(Movement::Vector3(endPoint[2], endPoint[0], endPoint[1]));
			}
...
void PathFinder::BuildPointPath(const float* startPoint, const float* endPoint)
{
	...
	{
		m_pathPoints[i] = Movement::Vector3(pathPoints[i * VERTEX_SIZE + 2], pathPoints[i * VERTEX_SIZE], pathPoints[i * VERTEX_SIZE + 1]);
	}
...
void PathFinder::createFilter(bool allowSwimming)
{
	unsigned short includeFlags = 0;
	unsigned short excludeFlags = 0;

	includeFlags |= NAV_GROUND;	
    excludeFlags |= (NAV_WATER | NAV_SLIME | NAV_MAGMA);

    //printf("include flag: %d \r\n", includeFlags);
    //printf("exclude flag: %d \r\n", excludeFlags);

	m_filter.setIncludeFlags(includeFlags);
	m_filter.setExcludeFlags(excludeFlags);

	// TODO
	updateFilter(allowSwimming, 0, 0, 0);
}
...
void PathFinder::updateFilter(bool isSwimming, float x, float y, float z)
{
	...
	if (isSwimming)
	{
        ...
        includedFlags = 0;
		includedFlags |= (NAV_WATER | NAV_GROUND);
...
bool PathFinder::HaveTile(const Movement::Vector3& p) const
{
    int tx, ty;
...
bool PathFinder::inRange(const Movement::Vector3& p1, const Movement::Vector3& p2, float r, float h) const
{
	Movement::Vector3 d = p1 - p2;
	return (d.x * d.x + d.y * d.y) < r * r && fabsf(d.z) < h;
}

float PathFinder::dist3DSqr(const Movement::Vector3& p1, const Movement::Vector3& p2) const
{
