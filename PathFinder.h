...
{
public:
	PathFinder(unsigned int mapId, unsigned int instanceId, bool allowSwimming);
	~PathFinder();
	...
	// result getters
	Movement::Vector3 getStartPosition()      const { return m_startPosition; }
	Movement::Vector3 getEndPosition()        const { return m_endPosition; }
	Movement::Vector3 getActualEndPosition()  const { return m_actualEndPosition; }
...
private:
	...
	Movement::Vector3        m_startPosition;    // {x, y, z} of current location
	Movement::Vector3        m_endPosition;      // {x, y, z} of the destination
	Movement::Vector3        m_actualEndPosition;// {x, y, z} of the closest possible point to given destination
	...
	void setStartPosition(const Movement::Vector3&point) { m_startPosition = point; }
	void setEndPosition(const Movement::Vector3&point) { m_actualEndPosition = point; m_endPosition = point; }
	void setActualEndPosition(const Movement::Vector3&point) { m_actualEndPosition = point; }
	...
	bool inRange(const Movement::Vector3& p1, const Movement::Vector3& p2, float r, float h) const;
	float dist3DSqr(const Movement::Vector3& p1, const Movement::Vector3& p2) const;
	...
	bool HaveTile(const Movement::Vector3& p) const;

	void BuildPolyPath(const Movement::Vector3& startPos, const Movement::Vector3& endPos);
	void BuildPointPath(const float* startPoint, const float* endPoint);
	void BuildShortcut();

	NavTerrain getNavTerrain(float x, float y, float z);
	void createFilter(bool allowSwimming);
