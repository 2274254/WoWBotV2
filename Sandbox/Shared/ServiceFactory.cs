...
namespace Agony.Sandbox.Shared
{
    public static class ServiceFactory
    {
        ...
        public static TInterfaceType CreateProxy<TInterfaceType>(string name = "") where TInterfaceType : class
        {
            try
            {
                return new ChannelFactory<TInterfaceType>(new NetNamedPipeBinding(), new EndpointAddress("net.pipe://localhost/" + PipeName + name)).CreateChannel();
            }
            catch (Exception e)
            {
                throw new Exception("Failed to connect to pipe for communication. The targetted pipe may not be loaded yet. Desired interface: " + typeof(TInterfaceType).Name, e);
            }
        }

        public static ServiceHost CreateService<TInterfaceType, TImplementationType>(bool open = true, string name = "") where TImplementationType : class
        {
            if (!typeof(TInterfaceType).IsAssignableFrom(typeof(TImplementationType)))
            {
                throw new NotImplementedException(typeof(TImplementationType).FullName + " does not implement " + typeof(TInterfaceType).FullName);
            }
