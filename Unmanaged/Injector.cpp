...
Injector::Injector(std::string nameIn) :
processName(nameIn),
autoInject(false),
closeOnInject(false),
hasInjected(false),
...
Injector::Injector(std::string nameIn, bool autoInjectIn, bool closeOnInjectIn) :
processName(nameIn), 
...
DWORD Injector::GetProcessId()
{
	...
	const char* process = processName.c_str();
...
HRESULT Injector::ManualMap(std::string filePath)
{
	DLL = filePath;
	...
	if (!CheckValidProcessExtension(processName.c_str()))
	{
	...
	HMODULE ret = remoteLoader.LoadLibraryByPathIntoMemoryA(filePath.c_str(), false);
	if (!ret)
	{
...
BOOL Injector::LoadLibraryInject(std::string filePath)
{
	isManualMap = false;
	DLL = filePath;
	...
	if (!CheckValidProcessExtension(processName.c_str()))
	{
		MessageBoxA(0, "Invalid Process Name!", "Injectora", MB_ICONEXCLAMATION);
		isReady = false;
		return -1;
	}
	...
	if (strlen(filePath.c_str()) < 5)
	{
		MessageBoxA(0, "Select a DLL first!", "Injectora", MB_ICONEXCLAMATION);
		isReady = false;
		return 2;
	}
	...
	HMODULE returnedModule = remoteLoader.LoadLibraryByPathA(filePath.c_str());
	if (returnedModule)
	{
	...
	{
		MessageBoxA(0, "LoadLibraryByPathA Failed!", "Injectora", MB_ICONERROR);
		bRet = FALSE;
	}
