...
bool CRemoteCode::SetBaseDirectory()
{
	...
	if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, NULL, szAppDataPath)))
	{
		...
		*(++lastDirectorySlash) = '\0';
		...
		if (!Utils::DoesDirectoryExist(m_baseDir))
			CreateDirectoryA(m_baseDir, NULL);
...
void CRemoteCode::DebugShout(const char *fmt, ...)
{
	...
	if (!doneOnce)
	{
		if (SetBaseDirectory())
		{
			...
			OutputDebugStringA(outTimeBuffer);
	...
	OutputDebugStringA(szNewLogBuffer);
...
void CRemoteCode::DebugShoutBufferHex()
{
	...
	minifiedBuf.append("\n\n");
	OutputDebugStringA(minifiedBuf.c_str());
}
#endif
