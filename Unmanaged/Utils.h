...
namespace Utils
{
	...
	static inline int numElementsInArray(Type(&array)[N])
	{
	...
	static void* GetProcAddress(HMODULE module, const char *proc_name)
	{
		...
		if ((char *)address >= (char*)exp_dir && (char*)address < (char*)exp_dir + exp_entry->Size)
		{
			...
			*func_name++ = 0;
			...
			if (!frwd_module)
			{
				MessageBoxA(0, "GetProcAddress failed to load module using GetModuleHandle and LoadLibrary!", "injectora", MB_ICONERROR);
				return NULL;
			}
